...
const SystemRolePromptCard: React.FC<SystemRolePromptCardProps> = ({ systemRolePrompt }) => {
  return (
    <Card title={`Role: ${systemRolePrompt.systemRole}`} style={{ marginBottom: '16px' }}>
      <p><strong>ID:</strong> {systemRolePrompt.id}</p>
      <p><strong>Prompt:</strong> {systemRolePrompt.systemPrompt}</p>
    </Card>
  );
};
...
const SystemRoleList: React.FC<SystemRolePromptGridProps> = ({ systemRolePrompts }) => {
  return (
    <Row gutter={16}>
